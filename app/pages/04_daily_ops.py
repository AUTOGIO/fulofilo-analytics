...
DISABLED_WRITEBACK_MSG = (
    "Sale deletion is intentionally disabled in the production dashboard. "
    "Canonical corrections must be made in data/excel/FuloFilo_Master.xlsx and synced."
)
...
# ── Manual sales entry ────────────────────────────────────────────────────────
st.markdown('<div class="ff-section-label">Manual Sales Capture</div>', unsafe_allow_html=True)
...
if products_df.empty:
    ...
    with st.form("manual_sale_form", clear_on_submit=False):
        ...
        submitted_sale = st.form_submit_button("Save Sale", use_container_width=True)
...
st.markdown('<div class="ff-section-label">Product Lookup</div>', unsafe_allow_html=True)
try:
...
st.divider()
with st.expander("Delete Sale Control", expanded=False):
