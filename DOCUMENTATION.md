...
FulôFiló Analytics Pro é um sistema de Business Intelligence local desenvolvido exclusivamente para a loja **FulôFiló** — varejo de souvenirs, roupas e acessórios regionais nordestinos. O sistema transforma dados brutos exportados do PDV Eleve Vendas em análises acionáveis: classificação ABC de produtos, análise de margem, controle de estoque e monitoramento de vendas diárias.
...
| Categorias de produtos inconsistentes no PDV | Motor de categorização com 30+ regras + interface de correção manual |
...
┌─────────────────────────────────────────────────────────────┐
│                        FONTES DE DADOS                       │
│  Eleve Vendas (JSON)  ·  inventory_TEMPLATE.csv  ·  PDV     │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────────────────────┐
...
|---|---|---|---|
| `build_catalog.py` | Constrói o catálogo mestre de 48 SKUs com custos, preços e ABC | `dashboard_data.json` | `products.parquet`, `product_catalog.csv` |
| `ingest_eleve.py` | Importa export do Eleve Vendas, valida schema, gera parquets de relatórios | `dashboard_data.json` | `revenue_report.parquet`, `profit_report.parquet`, `quantity_report.parquet` |
...
### Exportar dados do Eleve Vendas

1. No painel Eleve Vendas, acesse **Relatórios**
2. Exporte os três relatórios necessários:
   - Receita por produto (`revenue_report`)
   - Quantidade por produto (`quantity_report`)
   - Lucro por produto (`profit_report`)
3. O export deve estar no formato JSON com a seguinte estrutura:

```json
{
  "revenue_report":  [{"item": "...", "quantity": 0.0, "revenue": 0.0}],
  "quantity_report": [{"cost": 0.0, "item": "...", "profit": 0.0, "quantity": 0.0, "revenue": 0.0}],
  "profit_report":   [{"code": "...", "item": "...", "quantity": 0.0, "total": 0.0}]
}
```

4. Salve o arquivo como `dashboard_data.json`

### Colocar o arquivo no lugar correto

```
Destino: /Users/eduardogiovannini/dev/products/FuloFilo/data/raw/dashboard_data.json
```

**Atalho automático:** Se houver um arquivo `.json` em `~/Documents/FuloFilo_Inbox/`, o script `refresh_data.sh` o detecta e move automaticamente para `data/raw/`.
...
O script executa os 3 passos em sequência com log em `logs/refresh.log` e envia notificação macOS ao concluir.

**Para rodar apenas um passo específico:**
```bash
source .venv/bin/activate

# Apenas catálogo (após editar CATALOG em build_catalog.py)
python3 etl/build_catalog.py

# Apenas ingestão Eleve (novo dashboard_data.json)
python3 etl/ingest_eleve.py

# Apenas categorização
python3 etl/categorize_products.py

# Apenas estoque (após editar inventory_TEMPLATE.csv)
python3 -c "
import polars as pl
...
| **Fonte** | `products.parquet` ← `revenue_report` do Eleve Vendas |
| **Significado** | Soma de toda receita bruta gerada por vendas no período exportado |
| **Interpretação** | Representa o faturamento total. Não desconta devoluções nem impostos |
| **Atenção** | O período reflete os dados exportados do Eleve — não é MTD automático |
...
| **Fonte** | `quantity_report` do Eleve Vendas |
...
| **Fonte** | Campo `profit` do `quantity_report` do Eleve |
...
## 4.3 Atualização Semanal de Dados (15–20 min)

```
1. Exportar novo dashboard_data.json do Eleve Vendas
2. Copiar para data/raw/dashboard_data.json
3. Atualizar current_stock em data/raw/inventory_TEMPLATE.csv com contagem física
4. Rodar pipeline:
      bash scripts/refresh_data.sh
5. Verificar log: tail -20 logs/refresh.log
6. Abrir dashboard e conferir KPIs atualizados
7. Opcionalmente: gerar Excel via página Export Excel para arquivo histórico
```
...
## 5.3 `ingest_eleve.py` falha com "Input file not found"

**Causa:** `data/raw/dashboard_data.json` não existe.

**Solução:**
1. Exportar o JSON do Eleve Vendas
2. Salvar em `data/raw/dashboard_data.json`
3. Verificar que o arquivo tem as 3 chaves: `revenue_report`, `quantity_report`, `profit_report`

---

## 5.4 `ingest_eleve.py` falha com "Missing required keys"

**Causa:** O JSON exportado não tem o schema correto.

**Diagnóstico:**
```bash
python3 -c "
import json
d = json.load(open('data/raw/dashboard_data.json'))
print('Chaves encontradas:', list(d.keys()))
"
```

**Solução:** Verificar no Eleve Vendas se todos os três relatórios foram incluídos no export.
...
|---|---|
| `current_stock` no inventory_TEMPLATE.csv | Semanalmente (ou após grande reposição) |
| `dashboard_data.json` do Eleve | Semanalmente (ou mensalmente para análise de período) |
...
*FulôFiló Analytics Pro · Documentação interna · v1.0 · 2026*
