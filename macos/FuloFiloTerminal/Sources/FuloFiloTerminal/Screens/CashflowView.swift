...
struct CashflowView: View {
    ...
    private var categories: [String] {
        let base = Set(bubblePoints().map(\.category))
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: TerminalSpacing.sm) {
                ...
                SectionPanel(
                    ...
                    BubbleMarginChart(points: bubblePoints(), category: category)
    ...
    private func bubblePoints() -> [BubblePoint] {
        guard let snap = store.snapshot, !snap.bubbles.isEmpty else { return MockData.bubblePoints }
        return snap.bubbles.map {
            BubblePoint(sku: $0.sku, category: $0.category, volume: $0.volume, margin: $0.margin, revenue: $0.revenue)
        }
    }
}
